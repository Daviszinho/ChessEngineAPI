...
class ObsidianAdapter extends ChessEngineAdapter {
    constructor() {
        const envPath = process.env.OBSIDIAN_PATH;
        const candidates = [
            envPath,
            path.join(__dirname, '../../engines/obsidian'),
            '/usr/games/obsidian',
            'obsidian'
        ].filter(Boolean);
        const resolvedPath = candidates.find(p => p.includes('/') ? fs.existsSync(p) : true) || 'obsidian';

        super(resolvedPath);
        this.engineName = 'Obsidian';
    }

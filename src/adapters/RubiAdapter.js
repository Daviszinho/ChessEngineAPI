...
class RubiAdapter extends ChessEngineAdapter {
    constructor() {
        const envPath = process.env.RUBI_PATH;
        const candidates = [
            envPath,
            path.join(__dirname, '../../engines/RubiChess-20240817_x86-64'),
            '/usr/games/RubiChess',
            'RubiChess'
        ].filter(Boolean);
        const resolvedPath = candidates.find(p => p.includes('/') ? fs.existsSync(p) : true) || 'RubiChess';

        super(resolvedPath);
        this.engineName = 'Rubi';
    }

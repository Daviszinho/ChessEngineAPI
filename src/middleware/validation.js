...
const moveRequestSchema = Joi.object({
    ...
    engine: Joi.string()
        .valid('stockfish', 'reckless', 'torch-2', 'gnuchess', 'fruit', 'toga2', 'phalanx', 'crafty', 'glaurung')
        .default('stockfish')
        .messages({
            'any.only': 'Invalid engine. Available: stockfish, reckless, torch-2, gnuchess, fruit, toga2, phalanx, crafty, glaurung'
        }),
    level: Joi.number()
        .integer()
        .min(1)
